...
public class NpgsqlDataReader : NpgsqlDataReaderOrig
{
    ...
    public override bool IsDBNull(int ordinal)
    {
        ...
        if (ordinal < 0 || ordinal >= IsNull.Length)
            throw new IndexOutOfRangeException($"Column ordinal '{ordinal}' is out of range (0-{IsNull.Length - 1}).");

        return IsNull[ordinal] != 0;
    }
    ...
    protected override T GetFieldValueCore<T>(int ordinal)
    {
        var f = RowDescription?._fields[ordinal];
    ...
    /// </summary>
    internal override async Task Close(bool async, bool connectionClosing, bool isDisposing)
    {
    ...
    public override bool NextResult()
    {
        ...
        if (ResultIndex < SPITupleTables.Count)
        {
            ...
            _recordsAffected = ProcessedRows[ResultIndex];
